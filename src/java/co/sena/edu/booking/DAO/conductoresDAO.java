...
//            }
 ...
 public String conductores(String Id, Connection cnn) throws SQLException {
     ...
     try {
            ...
            while (rs.next()) {
                String idRes = Integer.toString(rs.getInt("idconductor") );
                HTMLTipos =HTMLTipos + " <option value='"+ rs.getInt("idconductor")+"' "+ (Id.equals(idRes) ? "selected":"") +" >"+ rs.getString("conductor") +"</option>";
            }
        } catch (SQLException ex) {
           mgSalida="a ocurrido un error" +ex.getMessage();
        }finally{
