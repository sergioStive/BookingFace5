...
//            }
 ...
 public String selecionTipoV(String Id, Connection cnn) throws SQLException {
     ...
     try {
            ...
            String query = "SELECT  idtipovehiculo as Id,CONCAT(tipovehiculo , ' - ' , MODELO,  ' - ' , marca)  as tipo "
                    + " FROM tipovehiculos ";
            pstmt = cnn.prepareStatement(query);
            rs = pstmt.executeQuery();
            ...
            while (rs.next()) {
                ...
                String idRes = Integer.toString(rs.getInt("id"));
                HTMLTipos =HTMLTipos + " <option value='"+rs.getInt("id")+"' "+ (Id.equals(idRes) ? "selected":"") +">"+ rs.getString("tipo") +"</option>";
            }
        } catch (SQLException ex) {
           msgSalida="a ocurrido un error" +ex.getMessage();
        }finally{
